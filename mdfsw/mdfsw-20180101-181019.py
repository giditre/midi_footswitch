...
def get_patch(s):
  global state_to_patch
  if s in [10, 11, 12, 13, 20, 21, 22, 23, 30, 31, 32, 33]:
    return state_to_patch[s]

def transition(s):
  '''
  There are 9 possible states: [11,12,13,21,22,23,31,32,33]
  The first digit represents LSW position, the second disgit represents last FSW pressed
  '''
  global dev_port
  global demo
  ...
  all_leds_off()
  new_patch_cmd = 'C0' + format(get_patch(s), '02x')
  if new_l == 1:
    gout(led1, out_True)
    if new_f == 1:
      gout(led4, out_True)
      mout(dev_port, new_patch_cmd, demo)
    elif new_f == 2:
      gout(led5, out_True)
      mout(dev_port, new_patch_cmd, demo)
    elif new_f == 3: 
